...
const EXACT_SOURCE_TABLES = new Set(
  DOMAIN_PROJECTION_DESCRIPTORS.map((descriptor) => descriptor.sourceQualifiedTable),
);
...
export interface DomainSourceIdentityIndex {
  contentHash(identityHash: DomainSourceSha256): Promise<DomainSourceSha256 | undefined>;
}
...
export class ClickHouseDomainSourceReader {
  constructor(
    ...
    if (!Number.isSafeInteger(lookbackMs) || lookbackMs < 0 || lookbackMs > 86_400_000) {
  ...
  async readPage(input: Readonly<{
    ...
    const checkpoint = input.checkpoint === null ? null : assertCursor(input.checkpoint);
    const start =
      checkpoint === null
        ? "1970-01-01T00:00:00.000Z"
        : new Date(Math.max(0, Date.parse(checkpoint.occurredAt) - this.lookbackMs)).toISOString();
    if (checkpoint !== null && compareTimestamp(checkpoint.occurredAt, readThrough) > 0) {
      throw sourceReaderError("DOMAIN_SOURCE_CURSOR_AHEAD");
    }
    const raw = await this.clickHouse.query(
      buildDomainSourcePageQuery(input.descriptor, start, readThrough, input.limit),
    ...
    for (const row of rows) {
      ...
      const persistedHash =
        localHash ?? (await this.identityIndex.contentHash(candidate.identityHash));
      ...
      if (nextCursor === null || compareDomainSourceCursor(candidate.cursor, nextCursor) > 0) {
...
export function compareDomainSourceCursor(
  ...
  const occurred = compareTimestamp(checkedLeft.occurredAt, checkedRight.occurredAt);
  if (occurred !== 0) return occurred;
  if (left.kind === "state") {
    const state = compareUInt64(left.stateSnapshotVersion!, right.stateSnapshotVersion!);
...
function sourceRecordFromRow(
  descriptor: DomainProjectionDescriptor,
  row: Record<string, unknown>,
): DomainSourceReadRecord {
  const sourceContractId = requiredString(row, "source_contract_id") as DomainSourceContractId;
  ...
  if (sourceContractVersion !== "1") throw sourceReaderError("SOURCE_CONTRACT_INVALID");
  ...
  const record: DomainSourceRecord = Object.freeze({
    ...
    ...(optional("task_id") === undefined ? {} : { taskId: optional("task_id") }),
    ...(optional("context_id") === undefined ? {} : { contextId: optional("context_id") }),
    agentId: requiredString(row, "agent_id"),
    ...(optional("agent_version") === undefined
      ? {}
      : { agentVersion: optional("agent_version") }),
    ...(optional("scenario_id") === undefined ? {} : { scenarioId: optional("scenario_id") }),
...
function assertCursor(value: DomainSourceCursor): DomainSourceCursor {
  ...
  if (value.recordId.trim() === "") throw sourceReaderError("DOMAIN_SOURCE_CURSOR_INVALID");
  if (value.kind === "state") {
    if (value.stateSnapshotVersion === null) throw sourceReaderError("DOMAIN_SOURCE_CURSOR_INVALID");
...
function parseRows(raw: string): Record<string, unknown>[] {
  ...
  return value["data"].map((row) => {
    if (!isRecord(row)) throw sourceReaderError("DOMAIN_SOURCE_RESPONSE_INVALID");
    return row;
  });
}
...
function fixedHash(value: unknown): DomainSourceSha256 {
  if (typeof value !== "string") throw sourceReaderError("SOURCE_CONTRACT_INVALID");
...
function decimalValue(value: unknown, field: string): string {
  ...
  throw sourceReaderError(field === "state_snapshot_version" ? "SOURCE_CONTRACT_INVALID" : "SOURCE_REVISION_INVALID");
}
...
function isStateDescriptor(descriptor: DomainProjectionDescriptor): boolean {
  return descriptor.definition.cursorPolicy.fields.includes("state_snapshot_version");
}

function assertPageLimit(value: number): void {
  if (!Number.isSafeInteger(value) || value < 1 || value > MAX_SOURCE_PAGE_SIZE) {
