...
export type DomainProjectionMetric = Readonly<{
  ...
  lagMs: number;
  openBlockingDeadLetters: number;
  schemaDrift: boolean;
  lastSuccessfulRunMs: number;
  leaseOwner: string;
  leaseExpiryMs: number;
  readySeals: number;
  expectedSeals: number;
}>;
...
export function createDomainProjectionWorkerApi(port: DomainProjectionRuntimeStatusPort): Server {
  return http.createServer((request: IncomingMessage, response: ServerResponse) => {
    void (async () => {
      ...
      response.end(JSON.stringify({ errorCode: "DOMAIN_WORKER_ROUTE_NOT_FOUND" }));
    })().catch(() => {
      if (!response.headersSent) response.writeHead(503, { "content-type": "application/json" });
      response.end(JSON.stringify({ errorCode: "DOMAIN_WORKER_DEPENDENCY_UNAVAILABLE" }));
    });
  });
}

export function domainProjectionRuntimeReady(snapshot: DomainProjectionRuntimeSnapshot): boolean {
  return snapshot.clickHouseReady && snapshot.controlPostgresReady && snapshot.schemaContractReady;
}

export function renderDomainProjectionMetrics(snapshot: DomainProjectionRuntimeSnapshot): string {
  const lines = [
  ...
  for (const metric of [...snapshot.projections].sort((left, right) => left.projectionId.localeCompare(right.projectionId))) {
    ...
    lines.push(`sdar_domain_projection_lease_info{${label},lease_owner="${escapeLabel(metric.leaseOwner)}"} 1`);
  }
  return `${lines.join("\n")}\n`;
}
...
function assertMetric(value: unknown): asserts value is number {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0) {
    throw Object.assign(new Error("DOMAIN_WORKER_METRIC_INVALID"), { code: "DOMAIN_WORKER_METRIC_INVALID" });
  }
}
